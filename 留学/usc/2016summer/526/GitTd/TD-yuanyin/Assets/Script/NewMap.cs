		...
		for (int row = 0; row < rows; row++) {
			for (int column = 0; column < columns; column++) {
				if (map [row, column] == 2) {
					...
					Vector3 position = new Vector3 ((float)(column - 15), 0.07f, -(float)(row) + 2);
					GameObject temp = (GameObject)Instantiate (barrel, position, this.transform.rotation);
					temp.name += ("_" + row + "_" + column);
				} else if (map [row, column] == 6) {
					Vector3 position = new Vector3 ((float)(column - 15), 0.07f, -(float)(row) + 2);
					GameObject temp = (GameObject)Instantiate (lid, position, this.transform.rotation);
					temp.name += ("_" + row + "_" + column);
				} else if (map [row, column] == 7) {
					Vector3 position = new Vector3 ((float)(column - 15), 0.07f, -(float)(row) + 2);
					GameObject temp = (GameObject)Instantiate (mushroom, position, this.transform.rotation);
					temp.name += ("_" + row + "_" + column);
				} else if (map [row, column] == 8) {
					Vector3 position = new Vector3 ((float)(column - 15), 0.07f, -(float)(row) + 2);
					GameObject temp = (GameObject)Instantiate (rock, position, this.transform.rotation);
					temp.name += ("_" + row + "_" + column);
