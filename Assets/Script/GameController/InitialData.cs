...
public class InitialData : MonoBehaviour
{
	...
	public float[] enemyhealth = new float[15];
	public int[] enemygold = new int[15];
	public int[] enemyscore = new int[15];
	public float[] enemyspeed = new float[15];

	// stat for tower and bullet (5 types of tower, 3 levels, 15 sets in total)
	// nameAB stands for: variable name, at type A, level B
	public float[,] range = new float[5,4];
	public int[,] cost = new int[5,4];
	public int[,] upgradeCost = new int[5,4];
	public float[,] turnSpeed = new float[5,4];
	public float[,] errorAmount = new float[5,4];
	public float[,] fireCooldown = new float[5,4];
	public int[,] fireCooldownLeft = new int[5,4];
	public float[,] rechargeRate = new float[5,4];

	//new added
	public float[,] elevateRate = new float[5,4];
	public float[,] elevateRadius = new float[5,4];

	public float[,] speed = new float[5,4];
	public float[,] damage = new float[5,4];
	public float[,] radius = new float[5,4];
	public int[,]	freeze=new int[5,4];
	public int[,]	poison=new int[5,4];
	public int[,] stun=new int[5,4];
	// stat for HUD
	public int gold = 0;
	public int hp = 0;
    ...
    void Awake() {
		...
		for (int j = 0; j < 4; j++) {
			for (int i = 0; i < 5; i++) {
				...
				range[i,j] = float.Parse(splits[0]);
				...
				freeze[i,j]=int.Parse(splits[13]);
				poison[i,j]=int.Parse(splits[14]);
				stun[i,j]=int.Parse(splits[15]);
