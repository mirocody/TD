...
//		}
		...
		if (transform.Find ("Wood"))
			woodMat = transform.Find ("Wood").GetComponent<Renderer> ().materials;
	...
	void Update () {
		...
		if (isTowerToBeInactive) {
			...
			// special case for fire-wood tower
			if (transform.Find ("Wood")) {
				transform.Find ("Wood").GetComponent<Renderer> ().materials [0] = transMat;
				transform.Find ("Wood").GetComponent<Renderer> ().materials [1] = transMat;
			}
...
//				transform.Find ("Tree").gameObject.SetActive (false);
		...
		if (isTowerToBeRestored)
		{
			...
			// special case for fire-wood tower
			if (transform.Find ("Wood"))
				transform.Find ("Wood").GetComponent<Renderer> ().materials = woodMat;
