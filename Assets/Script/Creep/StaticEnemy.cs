...
public class StaticEnemy : Enemy {
	...
	public void InstantiateNewGameObject() {
		...
		int choice = UnityEngine.Random.Range (80, 85);
        //int choice = 80;
    ...
    {
        Vector3 tempPostion = new Vector3(position.x, position.y + 1, position.z);
        if (choice >= InitialData.spotProbability + 4 * gap2)
        {
            temp = (GameObject)Instantiate(fireCard, tempPostion, this.transform.rotation);
        }
        else if(choice>= InitialData.spotProbability + 3 * gap2)
        {
            temp = (GameObject)Instantiate(woodCard, tempPostion, this.transform.rotation);
        }
        else if(choice>= InitialData.spotProbability + 2 * gap2)
        {
            temp = (GameObject)Instantiate(waterCard, tempPostion, this.transform.rotation);
        }
        else if (choice >= InitialData.spotProbability + gap2)
        {
            temp = (GameObject)Instantiate(earthCard, tempPostion, this.transform.rotation);
        }
        else
        {
            temp = (GameObject)Instantiate(metalCard, tempPostion, this.transform.rotation);
        }
